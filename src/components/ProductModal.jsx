...
export default function ProductModal({
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50 transition-opacity duration-200 animate-modal-in" role="dialog" aria-modal="true">
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6 w-full max-w-sm transform transition-all duration-200 scale-95 animate-modal-in" tabIndex="-1">
        ...
        <form onSubmit={async e => {
          ...
          <input name="model" type="text" placeholder={t.model || 'Model'} defaultValue={initialProduct?.model || ''} className="border border-gray-300 dark:border-gray-600 rounded px-2 py-1 dark:bg-gray-700 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-400" />
          <select name="category" defaultValue={initialProduct?.category || 'phones'} className="border border-gray-300 dark:border-gray-600 rounded px-2 py-1 dark:bg-gray-700 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-400">
